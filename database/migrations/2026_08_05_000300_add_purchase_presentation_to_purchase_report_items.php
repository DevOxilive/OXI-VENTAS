...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('purchase_report_items', function (Blueprint $table) {
            if (! Schema::hasColumn('purchase_report_items', 'purchase_presentation')) {
                $table->string('purchase_presentation', 20)->default('Pieza')->after('requested_quantity');
            }

            if (! Schema::hasColumn('purchase_report_items', 'package_quantity')) {
                $table->decimal('package_quantity', 12, 3)->nullable()->after('purchase_presentation');
            }

            if (! Schema::hasColumn('purchase_report_items', 'units_per_package')) {
    ...
    public function down(): void
    {
        Schema::table('purchase_report_items', function (Blueprint $table) {
            foreach (['units_per_package', 'package_quantity', 'purchase_presentation'] as $column) {
                if (Schema::hasColumn('purchase_report_items', $column)) {
