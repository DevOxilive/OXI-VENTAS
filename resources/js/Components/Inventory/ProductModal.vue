      ...
      <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div>
          ...
          <div class="space-y-3 max-h-[145px] overflow-y-auto pr-1">
            <div v-for="(barcode, index) in form.barcodes" :key="index" class="flex items-start gap-2">
              <div class="flex-1">
                <InputField label="" field="barcode" v-model="form.barcodes[index]" icon="barcode_scanner" :error="null"
                  :readonly="mode === 'view'" />
              </div>

              <button v-if="form.barcodes.length > 1 && mode !== 'view'" type="button" @click="removeBarcode(index)"
          ...
          <button v-if="mode !== 'view'" type="button" @click="addBarcode"
        ...
        <div>
          ...
          <div
            ...
            <template v-if="imagePreview">
              <img :src="imagePreview" :class="mode === 'view'
            ...
            <div v-if="mode !== 'view'" class="flex-1">
              ...
              <template v-if="mode !== 'view'">
                <input type="file" accept="image/*" class="mt-3 text-sm"
        ...
        <div v-if="mode !== 'view'" class="md:col-span-2">
          <div class="flex items-center justify-between mb-2">
            ...
            <button type="button" class="text-sm font-semibold text-slate-700 hover:text-black" @click="
          ...
          <div class="max-h-[130px] overflow-y-auto border border-slate-200 rounded-2xl p-3 bg-slate-50">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              <label v-for="branchItem in branchesDB" :key="branchItem.id"
                class="flex items-center gap-3 border rounded-xl px-4 py-3 bg-white cursor-pointer hover:bg-slate-100">
                <input type="checkbox" :value="branchItem.id" v-model="form.branch_ids"
                  :disabled="isCurrentBranch(branchItem.id)" class="rounded border-slate-300 disabled:opacity-60" />

                {{ branchItem.name }}
                <span v-if="isCurrentBranch(branchItem.id)" class="text-xs text-slate-400 ml-1">
                  actual
                </span>
              </label>
            </div>
        ...
        <SelectField label="Categoría" field="category_id" v-model="form.category_id" :error="form.errors.category_id"
          :disabled="mode === 'view'" :options="categoriesDB.map((category) => ({
            value: category.id,
            label: category.name,
          }))" placeholder="Selecciona categoría" />

        <InputField label="Nombre" field="name" v-model="form.name" :error="form.errors.name"
          :readonly="mode === 'view'" />

        <SelectField label="Unidad de medida" field="unit" v-model="form.unit" :options="units"
          placeholder="Selecciona unidad" :disabled="mode === 'view'" />

        <InputField label="Precio inicial" field="cost" v-model="form.cost" prefix="$" :error="form.errors.cost"
          type="text" step="0.01" :readonly="mode === 'view'" />

        <InputField label="Precio venta" field="sale_price" v-model="form.sale_price" prefix="$"
