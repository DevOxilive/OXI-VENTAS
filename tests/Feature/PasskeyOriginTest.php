...
class PasskeyOriginTest extends TestCase
{
    ...
    public function test_production_domain_uses_its_hostname_as_the_relying_party_id(): void
    {
        $request = Request::create('https://super-kay.com.mx/user/passkeys/options');

        app(ResolvePasskeyOrigin::class)->handle($request, fn () => response()->noContent());

        $this->assertSame('super-kay.com.mx', config('passkeys.relying_party_id'));
        $this->assertSame(['https://super-kay.com.mx'], config('passkeys.allowed_origins'));
    }
}
