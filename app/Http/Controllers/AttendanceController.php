...
class AttendanceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('HumanResources/Attendance', [
            ...
            'options' => [
                ...
                'departments' => $canViewAttendance ? Employee::query()->whereNotNull('department')->where('department', '!=', '')->distinct()->orderBy('department')->pluck('department')->map(fn ($department) => ['value' => $department, 'label' => $department]) : [],
    ...
    private function recordsQuery(Request $request, bool $canManage, array $filters) { return AttendanceRecord::query()->with(['user.role', 'employee', 'branch'])->when(!$canManage, fn ($query) => $query->where('user_id', $request->user()->id))->when($filters['from'] ?? null, fn ($query, $value) => $query->whereDate('attendance_date', '>=', $value))->when($filters['to'] ?? null, fn ($query, $value) => $query->whereDate('attendance_date', '<=', $value))->when($filters['branch'] ?? null, fn ($query, $value) => $query->where('branch_id', $value))->when($filters['department'] ?? null, fn ($query, $value) => $query->whereHas('employee', fn ($employee) => $employee->where('department', $value)))->when($filters['employee'] ?? null, fn ($query, $value) => $query->where('employee_id', $value))->when($filters['type'] ?? null, fn ($query, $value) => $query->where('type', $value))->latest('recorded_at'); }
